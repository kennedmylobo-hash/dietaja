...
const sendEmailNotification = async (
  ...
  branding: TenantBranding, baseUrl: string
) => {
  ...
  try {
    const response = await resend.emails.send({
      ...
      to: [email],
      subject: replaceVariables(subject, order, coupon, discount, baseUrl),
      html,
    });
      ...
      for (const order of orders) {
        ...
        try {
          ...
          await sendEmailNotification(order.customer_email, order as OrderData, template.email_subject, template.email_body_html,
            template.coupon_code, template.discount_percent, branding, baseUrl);
