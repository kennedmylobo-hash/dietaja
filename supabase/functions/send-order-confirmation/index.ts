...
const generateEmailHtml = (data: OrderConfirmationRequest, brandName: string, whatsapp: string, whatsappFormatted: string, baseUrl: string, pickupNeighborhood: string): string => {
  ...
  const deliveryInfo = data.delivery_option === 'pickup' 
    ? `📍 Retirada no local - ${pickupNeighborhood}`
    ...
    <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
      <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
        ...
        <div style="padding: 24px;">
          <h3 style="margin: 0 0 12px; font-size: 14px; color: #666; text-transform: uppercase;">Dados do Cliente</h3>
        ...
        <div style="padding: 0 24px 24px;">
          <h3 style="margin: 0 0 12px; font-size: 14px; color: #666; text-transform: uppercase;">Itens do Pedido</h3>
          <table style="width: 100%; border-collapse: collapse; background-color: #fafafa; border-radius: 8px; overflow: hidden;">${itemsHtml}</table>
        </div>
        <div style="margin: 0 24px 24px; padding: 16px; background-color: #f9fafb; border-radius: 8px;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px;"><span style="color: #666;">Subtotal</span><span>R$ ${data.subtotal.toFixed(2).replace('.', ',')}</span></div>
          ${data.delivery_fee > 0 ? `<div style="display: flex; justify-content: space-between; margin-bottom: 8px;"><span style="color: #666;">Taxa de Entrega</span><span>R$ ${data.delivery_fee.toFixed(2).replace('.', ',')}</span></div>` : ''}
          <div style="display: flex; justify-content: space-between; padding-top: 12px; border-top: 1px solid #e5e7eb; font-size: 18px; font-weight: bold;"><span>Total a Pagar</span><span style="color: #d97706;">R$ ${data.total.toFixed(2).replace('.', ',')}</span></div>
        </div>
        <div style="padding: 0 24px 32px; text-align: center;">
          <p style="margin: 0 0 16px; font-size: 14px; color: #666;">Prefere pagar pelo WhatsApp?</p>
          <a href="https://wa.me/${whatsapp}?text=${encodeURIComponent(`Olá! Quero finalizar o pagamento do meu pedido #${data.order_number}`)}" style="display: inline-block; padding: 12px 24px; background-color: #25D366; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: bold;">💬 Pagar via WhatsApp</a>
        </div>
        <div style="padding: 24px; background-color: #f9fafb; text-align: center; border-top: 1px solid #e5e7eb;">
          ...
          <p style="margin: 0; font-size: 12px; color: #999;">Este é um e-mail automático. Por favor, não responda.</p>
        </div>
      </div>
    </body>
    </html>
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { data: emailResponse, error } = await resend.emails.send({
      from: `${branding.brand_name} <noreply@pedidoja.com.br>`,
    ...
    if (error) {
      ...
      await supabase.from('notification_events').insert({
        channel: 'email', event_type: 'failed', order_number: data.order_number,
        recipient_email: data.customer_email, template_name: 'order_confirmation',
        tenant_id: data.tenant_id, metadata: { error: error.message }
      });
      throw error;
    }
    ...
    await supabase.from('notification_events').insert({
      channel: 'email', event_type: 'sent', order_number: data.order_number,
      recipient_email: data.customer_email, template_name: 'order_confirmation',
