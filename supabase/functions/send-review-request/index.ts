...
const sendWhatsAppNotification = async (phone: string, message: string, orderNumber: string) => {
  ...
  try {
    ...
    console.log(`Sending WhatsApp review request to ${formattedPhone} for order ${orderNumber}`);
...
const sendEmailNotification = async (email: string, order: OrderData, subject: string, bodyHtml: string) => {
      ...
      <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        ...
        <div style="padding: 30px;">
          ...
          <div style="font-size: 16px; color: #666; margin-bottom: 25px;">
            ${replaceVariables(bodyHtml, order)}
          </div>
          ...
          <div style="text-align: center;">
            <a href="https://wa.me/5577991001658?text=Olá! Quero avaliar meu pedido ${order.order_number}" style="display: inline-block; background: #22c55e; color: white; text-decoration: none; padding: 15px 40px; border-radius: 8px; font-weight: bold; font-size: 16px;">
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (templateError || !template?.is_active) {
      console.log("Review request template not found or inactive");
      return new Response(
    ...
    // Find delivered orders that haven't received review request
    const { data: orders, error: ordersError } = await supabase
      .from("orders")
      .select("id, order_number, customer_name, customer_email, customer_phone, total, delivered_at")
      .eq("status", "delivered")
    ...
    console.log(`Found ${orders?.length || 0} orders eligible for review request`);
    ...
    for (const order of orders) {
      try {
        // Send WhatsApp
        const whatsappMessage = replaceVariables(template.whatsapp_template, order as OrderData);
