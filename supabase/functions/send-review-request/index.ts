...
const sendEmailNotification = async (email: string, order: OrderData, subject: string, bodyHtml: string, branding: TenantBranding, baseUrl: string) => {
      ...
      <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        ...
        <div style="padding: 30px;">
          ...
          <div style="text-align: center;">
            <a href="https://wa.me/${branding.whatsapp}?text=Olá! Quero avaliar meu pedido ${order.order_number}" style="display: inline-block; background: #22c55e; color: white; text-decoration: none; padding: 15px 40px; border-radius: 8px; font-weight: bold; font-size: 16px;">
    ...
    for (const order of orders) {
      try {
        ...
        if (!brandingCache[tenantKey]) {
          ...
          brandingCache[tenantKey] = { branding: b, baseUrl: getTenantBaseUrl(b) };
        }
        ...
        await sendEmailNotification(order.customer_email, order as OrderData, template.email_subject, template.email_body_html, branding, baseUrl);
